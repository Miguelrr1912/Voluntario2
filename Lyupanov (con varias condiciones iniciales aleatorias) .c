...
int main (void)
{
    ...
    file3 = fopen("Lyupanov5condinicdif.txt", "w");
    if (file3 == NULL) {
        printf("Error opening file!\n");
        return 1;
    }
    ...
    cuerpo.thetha=(double)(rand()%100)/400.0; //Genera un ángulo aleatorio entre 0 y 0.5 radian
    cuerpo.phi=(double)(rand()%100)/400.0;
